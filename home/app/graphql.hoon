...
=<
  ...
  |_  =bowl:gall
    ++  poke-handle-http-request  
      ...
      ?~  body.request.req
        ~&  %invalid-post-no-body
          !!
      =+  db-ip=(tail (~(get by env-vars) %db-ip))
    ...
    ++  parse-graphql-request
      ...
      =/  query-tape  (trip state(query (~(got by query) eyre-id)))
      ...
      ?:  =(query-or-mutation "mutation")
        (give-simple-payload:app eyre-id [[200 ~] [~ (as-octt:mimes:html (trip (process-mutation [my-body-tape replace])))]])

      ::  If it's a query, go to the process-query arm
      ?:  =(query-or-mutation "query")
        (give-simple-payload:app eyre-id [[200 ~] [~ (as-octt:mimes:html (trip (process-query [query-or-mutation-name replace payload])))]])

      ::  Else return an error (just a cord rather than formal error for now)
      (give-simple-payload:app eyre-id [[200 ~] [~ (as-octt:mimes:html "Request is not valid")]])
    ++  process-query
      |=  [query-name=tape replace=tape payload=simple-payload:http]
      ^-  @t
      ...
      ::  TODO: below, there should be a conditional for each valid query. This should later be abstracted to a separate file
      ?:  =((head (tail query-resolver-output)) "getBooks")
        =+  book-resolver-output=(gql-schema-book [query-resolver-output payload])
